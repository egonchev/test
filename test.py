[Unit]
Description=Timer for the mysqlsh backup service which managers our DB backups

[Timer]
Unit=mysqlsh-backup.service
OnCalendar=*:0/15



[Unit]
Description=Run the MysqlSh backup
After=mysql.service

[Service]
Type=simple
ExecStart=/usr/bin/python3 /opt/mysqlsh-backup/mysqlsh-backup.py


https://gist.github.com/ebta/c4fdbe922d9a9ec2e258c659cbb52786
https://dev.mysql.com/blog-archive/mysql-shell-dump-load-part-1-demo/
https://www.percona.com/blog/various-backup-compression-methods-using-mysqlpump/
 https://www.red-gate.com/simple-talk/blogs/backing-up-mysql-part-3-mysqlpump/
https://blog.opstree.com/category/percona/
  https://www.percona.com/blog/perconalab-autoxtrabackup-v1-5-0-release/
    https://www.percona.com/blog/the-mysqlpump-utility/
https://dev.mysql.com/doc/refman/8.0/en/mysqlpump.html
  https://dasini.net/blog/2020/12/17/mysql-shell-the-new-era/
    https://dev.to/nejremeslnici/mysql-shell-the-best-tool-for-your-logical-backups-44fk
      https://severalnines.com/blog/logical-database-backups-using-mysql-shell/
        https://docs.oracle.com/cd/E17952_01/mysql-shell-8.0-relnotes-en/news-8-0-27.html
...
#                raise Exception("Pexpect timed out waiting for password prompt. G4L3JRBL")
            ...
            self._log(msg="Waiting for child process to finish executing...", lvl=LogLvl.TRACE)
            ...
            while True:
                try:
                    ...
                    self._log(msg="Unexpected exception waiting for child process to finish.", e=ex, lvl=LogLvl.TRACE)
            ...
            if pexpect_process.exitstatus != 0:
                self._log(msg="Failed to create 'Full Backup'. Non-zero return code. SDDBDCIM", lvl=LogLvl.TRACE)
                raise Exception("Failed to create 'Full Backup'. Non-zero return code. SDDBDCIM")
        except Exception as ex:
            self._log(msg="Failed to execute command 'Full Backup' 4YYQ6AEV", e=ex, lvl=LogLvl.TRACE)
            ...
            if pexpect_process is not None:
                self._log(msg="Closing spawned child process. ProcessID: [{pid}].".format(pid=pexpect_process.pid), lvl=LogLvl.TRACE)
                pexpect_process.close()
